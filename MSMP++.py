...
path_results_baseline = "C:/Users/quint/Documents/Master 2025-2026/Block 2/Computer Science for Econometrics/Individual assignment/Assignment code/MSMP+ results/test_results.json"
...
# Data cleaning
print("Cleaning data...")
data = clean_data(data, clean_text_improved)
print(f"-> Loaded and cleaned {len(data)} product entries.")

# Binary representation for LSH
print("Creating the binary vectors...")
relevant_features = extract_relevant_features(data, len(data)*0.10)
binary_vectors = build_binary_vectors_improved(data, relevant_features)
...
for b in range(n_bootstraps):
    ...
    # True pairs for evaluation (include pair if both entries are in corresponding indices set)
    true_pairs_train = [
...
with open(path_results_baseline, "r", encoding="utf-8") as f: 
...
# Convert baseline_json to pd dataframe
baseline_df = pd.DataFrame(baseline_json)
baseline_df["(r,b)"] = baseline_df["(r,b)"].apply(tuple) # Convert list to tuple for .agg
...
plt.plot(baseline_summary_df["foc_lsh"], baseline_summary_df["pc_lsh"], label='MSMP+', color='grey')
plt.plot(summary_df["foc_lsh"], summary_df["pc_lsh"], label='MSMP++', color='black')
...
# Pair Quality LSH
plt.figure(figsize=(8,5))
plt.plot(baseline_summary_df["foc_lsh"], baseline_summary_df["pq_lsh"], label='MSMP+', color='grey')
plt.plot(summary_df["foc_lsh"], summary_df["pq_lsh"], label='MSMP++', color='black')
plt.xlabel("Fraction of comparisons", fontsize=12)
...
# F1 star
plt.figure(figsize=(8,5))
plt.plot(baseline_summary_df["foc_lsh"], baseline_summary_df["f1_star"], label='MSMP+', color='grey')
plt.plot(summary_df["foc_lsh"], summary_df["f1_star"], label='MSMP++', color='black')
plt.xlabel("Fraction of comparisons", fontsize=12)
...
# F1 MSM
plt.figure(figsize=(8,5))
plt.plot(baseline_summary_df["foc_lsh"], baseline_summary_df["f1"], label='MSMP+', color='grey')
plt.plot(summary_df["foc_lsh"], summary_df["f1"], label='MSMP++', color='black')
plt.xlabel("Fraction of comparisons", fontsize=12)
plt.ylabel("F1-measure", fontsize=12)
plt.legend(loc='upper right', frameon=False, fontsize=10)
plt.ylim(bottom=0)
...
# --------------- Compute area under the graphs ---------------

def compute_fair_auc(df1, df2, x_col, y_col):

    # Drop NaNs and sort
    t1 = df1[[x_col, y_col]].dropna().sort_values(x_col)
    t2 = df2[[x_col, y_col]].dropna().sort_values(x_col)

    # Find shared max foc_lsh for fair comparison
    xmax = min(t1[x_col].max(), t2[x_col].max())

    # Restrict to shared range
    t1 = t1[t1[x_col] <= xmax]
    t2 = t2[t2[x_col] <= xmax]

    # Compute AUC
    auc1 = np.trapezoid(t1[y_col].values, t1[x_col].values)
    auc2 = np.trapezoid(t2[y_col].values, t2[x_col].values)
    return auc1, auc2

auc_pc_baseline, auc_pc_msmp2 = compute_fair_auc(
    baseline_summary_df, summary_df, "foc_lsh", "pc_lsh"
)

auc_pq_baseline, auc_pq_msmp2 = compute_fair_auc(
    baseline_summary_df, summary_df, "foc_lsh", "pq_lsh"
)

auc_f1_baseline, auc_f1_msmp2 = compute_fair_auc(
    baseline_summary_df, summary_df, "foc_lsh", "f1_star"
)

print("========== AREA UNDER CURVE RESULTS ==========")

print("MSMP+ (baseline)")
print(f"AUC PC  : {auc_pc_baseline:.4f}")
print(f"AUC PQ  : {auc_pq_baseline:.4f}")
print(f"AUC F1* : {auc_f1_baseline:.4f}\n")

print("MSMP++")
print(f"AUC PC  : {auc_pc_msmp2:.4f}")
print(f"AUC PQ  : {auc_pq_msmp2:.4f}")
print(f"AUC F1* : {auc_f1_msmp2:.4f}\n")